...
export function processText(inputText: string, arrays: Array<Array<string> | Array<Citation>>): ProcessTextReturn {
  ...
  inputText = inputText.replaceAll("**", ""); // Remove bold markdown
  ...
  let replacedText = inputText.replace(findCitations, (_match, citationName, citationLink) => {
    const citationText = citationName.trim();
    if (!citation[citationText]) {
      citation[citationText] = referenceCounter++;
      citationLinks[citationText] = citationLink;
    }
    return ``;
  });
  ...
  // Remove each element from nextQuestions surrounded by << and >> from replacedText
  nextQuestions.forEach((question) => {
    const questionWithArrows = `<<${question}>>`;
    replacedText = replacedText.replace(questionWithArrows, '');
  });
  ...
  replacedText = stepsIndex !== -1 ? replacedText.substring(0, stepsIndex + 4) : replacedText;
